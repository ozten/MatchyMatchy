...
testbed/
  ...
  pairs/                  # Tier 3 — real-pair regression fixtures (tracked, hermetic, CI-gated)
    p01-hiya-number-registration/
      pair.json            # provenance + SHA-256 integrity + expectedState (green|red)
      expected-issues.json # hand-authored INTENT (reuses the variant matcher DSL verbatim)
      desktop/             # <viewport> subdir — analyze resolves screenshots relative to its parent
        old.bundle.json    # frozen capture (NEVER regenerated by verify)
        new.bundle.json
        old.png / old-vp.png / new.png / new-vp.png   # frozen screenshots — analyze HARD-REQUIRES these
  check-pair.py           # Tier-3 replay+integrity+xfail harness (reuses check-fixture.py's engine)
  pair-add.py             # capture → privacy gate → freeze → scaffold
  pair_privacy.py         # credential/PII/ownership freeze gate (fail-closed on token-shaped params)
...
### Tier 3 — real-pair regression fixtures (`testbed/pairs/`)

The convenience tier for the *"I hit a real old/new URL where matchy misses a defect or floods noise — add it to the bench, fix the tool"* loop. A pair is captured once (`make pair-add CASE=… URL_OLD=… URL_NEW=…`), frozen (bundles + screenshots committed under `<viewport>/`), and asserted hermetically forever after (`make pair CASE=…` / the Tier-3 step in `make verify`) — `matchy analyze` replays the frozen bundles with **no Chromium, Playwright, testbed servers, or network**. Conventions:

- **`<viewport>/` nesting is load-bearing.** `matchy analyze` resolves screenshots as `bundle.parent().parent()/screenshots.fullPage` and hard-errors on a missing PNG, so bundles freeze at `pairs/<case>/<viewport>/old.bundle.json` with the PNGs alongside. A flat freeze breaks replay.
- **`expectedState: green | red`.** `red` locks a defect matchy cannot yet handle — `check-pair.py` reports it as a gate-safe **XFAIL** (exit 0), so a "commit red, fix later" TDD entry does not break `main`. `green` is a passing guard. (Distinct from `demonstrates`, which records the *kind* of case: false-negative / false-positive / true-positive / mixed.) Flip red→green when the fix lands.
- **Privacy/ownership.** `pair-add.py` runs `pair_privacy.py` BEFORE freezing: an unconditional, fail-closed credential token-scan (cannot be bypassed by `--yes`) plus an interactive human PII-review + ownership-rights assertion (`--yes` skips only the interactive parts — trusted re-freezes only). Captures are committed to a PUBLIC repo permanently. Automated PII *detection* is roadmapped, not in M9.
- **Engine reuse (non-negotiable).** `check-pair.py` imports `check-fixture.py`'s matcher engine — the `expected-issues.json` DSL is shared verbatim; never a second implementation.
- **Adding a brand-new red fixture is NOT a golden change** (no `golden-auditor` sign-off needed). Re-recording bundles via `pair-refresh`, or later weakening an expectation, still is.
